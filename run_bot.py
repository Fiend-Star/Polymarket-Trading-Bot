...
class IntegratedBTCStrategy(Strategy):
    ...
    def _find_btc_instrument(self):
        ...
        if current_markets:
            ...
            logger.info(
                f"⚠ No current market, selecting next: {selected['slug']} (starts in {selected['time_diff_minutes']:.1f} min)")
    ...
    async def _make_trading_decision(self, current_price):
        ...
        logger.info(
            f"FUSED SIGNAL: {fused.direction.value} (score={fused.score:.1f}, confidence={fused.confidence:.2%})")
...
def run_integrated_bot(simulation: bool = True, enable_grafana: bool = True, test_mode: bool = False):
    ...
    logger.info(
        f"  Expiring between: {now.strftime('%H:%M:%S')} - {(now + timedelta(minutes=30)).strftime('%H:%M:%S')} UTC")
